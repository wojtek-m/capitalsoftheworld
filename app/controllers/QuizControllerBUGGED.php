    ...
    public function generateQuiz($number_of_questions) {
        ...
        if ($number_of_questions > $max_questions) {
            return View::make('quizz.quizz-sorry', array('max' => $max_questions))
                ->with('title', 'Quiz Size Error | '); 
        } 

        $all_capitals_temp = Country::where('id', '>=', 0)->select('capital')->get();
        $random_list = array();

        //$random_list = range(1,sizeof($all_capitals_temp));
        //shuffle($random_list);
        ...
        $randomizer_250 = range(1,sizeof($all_capitals_temp));
        shuffle($randomizer_250);

        // Put all capitals / ID's in the database in the arrays
        $all_capitals = array();
        foreach ($all_capitals_temp as $index => $capital) {
            // capitals array
            $all_capitals[$index] = $all_capitals_temp[$index]['capital'];
            // array with ID's
            $random_list[$index] = $all_capitals_temp[$index]['id'];
        }

        var_dump($random_list);
        shuffle($random_list);
        ...
        var_dump(sizeof($all_capitals_temp));
        //var_dump($questions_temp);
        //foreach ($questions_temp as $index => $question) {
        //    echo $questions_temp[$index][0]['capital'];
        //}

        // put capital / country pairs in arrays to avoid passing objects to session.
        foreach ($questions_temp as $index => $question) {
            //if (array_key_exists(0, $questions_temp[$index])) {
